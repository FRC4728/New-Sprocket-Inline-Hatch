...
c2_1.whileTrue(new InstantCommand(() -> s_Arm.ManualUp()));
c2_3.whileTrue(new InstantCommand(() -> s_Extend.RetractButton()));
c2_4.whileTrue(new InstantCommand(() -> s_Arm.ManualDown()));
c2_7.whileTrue(new InstantCommand(() -> s_Arm.Stop()));
c2_5.onTrue(MidArmCube());
c2_10.onTrue(OhShitHome());
c2_6.onTrue(new ArmToHopperCommand(s_Arm).until (() -> (s_Arm.getEncoderActuate() > -11.4) & (s_Arm.getEncoderActuate() < -10.7)).andThen(new ArmStopCommand(s_Arm)));
c2_8.onTrue(new ArmToGroundCommand(s_Arm).until(() -> (s_Arm.getEncoderActuate() < 23.7) & (s_Arm.getEncoderActuate() > 23)));
